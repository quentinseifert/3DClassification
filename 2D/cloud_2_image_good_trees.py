...
n_trees, n_counts = get_data('../data/Nadelbäume', n_points=6000, k=3)
...
for i in range(0,l_trees.shape[0]):
    ...
    for ii in range(0, 360, 36):
        ...
        fig.savefig('Images/' + str(labels[i]) +'/' + 'laub' + '_' + str(i) + '_' + str(labels[i]) + "%d.png" % ii)
        plt.close(fig)
...
for i in range(0,n_trees.shape[0]):
    ...
    for ii in range(0, 360, 36):
        ax.view_init(elev=10., azim=ii)
        fig.savefig(str(labels[i]) +'/' + 'nadel' + '_' + str(i) + '_' + str(labels[i]) + "%d.png" % ii)
        plt.close(fig)
